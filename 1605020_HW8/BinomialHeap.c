...
{
  // if(list->tail)printf("tail = %d",list->tail->node->key);
  // if(list->tail->prev == NULL)printf(", tail->prev = NULL");
  // if(list->tail->next == NULL)printf(", tail->next == NULL");
  // printf("\n");
...
linked_list * union_heap(linked_list * heap1, linked_list * heap2)
{
  ...
  while(ptr1 != NULL && ptr2 != NULL)
  {
    ...
    else
    {
      ...
      else
      {
        ...
        {
          // printf("heap's_last.order = ptr.order\n");
  ...
  {
    tree_root * exptr = (ptr1 == NULL)? ptr2 : ptr1;
    while(exptr != NULL)
    {
      if(heap->tail->node->order == exptr->node->order)
      {
        tree_root * linked_root = binomial_link(heap->tail, exptr);
        // printf("tail...\n");print_list(heap);
        remove_last(heap);
        // printf("Extra\n");
        add_last(heap, linked_root);
...
Node * find_min(linked_list * list)
{
  tree_root * ptr = list->head;
  int min = 999999;
  Node * node;
...
int main()
{
  ...
  binomial_heap = insert(binomial_heap, 80);
